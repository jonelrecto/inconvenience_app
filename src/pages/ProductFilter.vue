<template>
  <q-page padding>
    <span v-if="products && products.length <= 0">No product search</span>
    <div class="row">
      <div class="col">
...
export default {
  created () {
    ...
    console.log(this.searchKey, 'serach key')
  },
  async mounted () {
    // await this.$binding('comments', this.$fbFirestore.collection('products').doc(this.productId).collection('comments'))  
    this.$binding('products', this.$fbFirestore.collection('products').where('title', '==', this.searchKey))
      .then(resp => {
  ...
  watch: {
    ['$route.params.searchKey']: {
      async handler () {
        this.searchKey = this.$route.params.searchKey
        console.log(this.searchKey, 'finding')
        await this.$binding('products', this.$fbFirestore.collection('products').where('title', '==', this.searchKey))
          .then(resp => {
            console.log(resp, 'prod resp watch')
          })
      },
      deep: true
    }
